...
def main():
    ...
    if not model.use_topology and feature_type == 'topo':
        print("❌ 当前配置 use_topology=False，不能提取 topo-only 特征。")
        return
    ...
    if feature_type == 'visual':
        ...
        output_dim = config['topo_dim']
    else:
        output_dim = config['feature_dim'] + config['topo_dim'] if model.use_topology else config['feature_dim']
