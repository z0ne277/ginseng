...
def create_model(config, device, logger):
    ...
    if config['use_topology']:
        enabled = []
        if config['use_legacy_branch']:
            enabled.append('Legacy')
        if config['use_skeleton_branch']:
            enabled.append('Skeleton')
        if config['use_edge_branch']:
            enabled.append('Edge')
        if config['use_frequency_branch']:
            enabled.append('Frequency')
        logger.log(f"Enabled branches: {', '.join(enabled)}\n")
    else:
        logger.log("Topology is disabled. Visual-only training mode.\n")
...
def create_optimizer_and_scheduler(model, config, logger):
    ...
    param_groups = []
    if backbone_params:
        param_groups.append({'params': backbone_params, 'lr': config['learning_rate'] * 0.1})
    if topo_params:
        param_groups.append({'params': topo_params, 'lr': config['learning_rate']})
    if other_params:
    ...
    logger.log(f"  Topology LR: {config['learning_rate']} (if topology params exist)")
...
def train_model(model, data_loaders, device, optimizer, scheduler, config, logger):
    ...
    for epoch in range(config['num_epochs']):
        ...
        if epoch < config['warmup_epochs'] and config['lr_scheduler'] != 'cosine_warmup':
            ...
            lr_group_idx = 1 if len(optimizer.param_groups) > 1 else 0
            current_lr = optimizer.param_groups[lr_group_idx]['lr']
...
def parse_args():
    ...
    # 支路配置
    parser.add_argument('--no_topology', action='store_true', help='Disable all topology modules')
    ...
    # 模型参数
    parser.add_argument('--topo_weight', type=float, default=0.15)
    parser.add_argument('--topo_dim', type=int, default=128)
    parser.add_argument('--feature_dim', type=int, default=256)

    # 其他
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints/hybrid_v3_13/')
...
def main():
    ...
    if 'use_topology' not in config:
        config['use_topology'] = True

    # 只有在命令行明确指定时才覆盖默认配置
    if args.no_topology:
        config['use_topology'] = False
        config['topo_weight'] = 0.0
    ...
    elif args.checkpoint_dir != 'checkpoints/hybrid_v3_13/':
    ...
    set_random_seed(config['seed'], logger)
    ...
    logger.log(f"Use topology: {config['use_topology']}")
    ...
    if config['use_topology']:
        ...
        logger.log("Branches: disabled (visual-only)")
    logger.log("=" * 70 + "\n")
