...
class SwinFeatureMap(nn.Module):
    """Expose the last Swin stage as a BCHW feature map."""
...
class ViTFeatureMap(nn.Module):
    """Convert ViT patch tokens back to their square spatial grid."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.reshape(batch, self.hidden_dim, height * width).permute(0, 2, 1)
        class_token = self.class_token.expand(batch, -1, -1)
        x = self.encoder(torch.cat([class_token, x], dim=1))[:, 1:]
...
def build_feature_backbone(
    ...
    """Build a spatial feature backbone for controlled backbone ablations."""
    ...
    if name == "swin_v2_t":
        ...
        backbone = models.swin_v2_t(weights=weights)
        return SwinFeatureMap(backbone), 768
    if name == "vit_b_16":
        weights = models.ViT_B_16_Weights.IMAGENET1K_V1 if pretrained else None
        backbone = models.vit_b_16(weights=weights)
        return ViTFeatureMap(backbone), 768
    raise ValueError(
        "unsupported backbone_name; expected one of "
        "resnet50, convnext_tiny, swin_v2_t, vit_b_16"
    )
...
class MorphologicalErosion(nn.Module):
    ...
    def __init__(
        ...
        if operator not in {"min", "max", "avg", "identity"}:
            raise ValueError("operator must be min, max, avg or identity")
...
class TopoFeatureExtractor(nn.Module):
    ...
    def __init__(
            ...
            topology_operator: str = "min"
    ):
        """
        Args:
            input_channels: 输入特征通道数 (ResNet50为2048)
            num_erosion_levels: 层级特征图数量，包含未腐蚀的 E0；
                最大实际腐蚀次数为 num_erosion_levels - 1
            topo_dim: 最终拓扑特征维度
        """
        super().__init__()
        ...
        self.topology_operator = topology_operator
...
class ImprovedMoCoV3WithTopoSideline(nn.Module):
    ...
    def __init__(
            ...
            erosion_kernel_size: int = 3,
            topology_operator: str = "min",
            topology_negative_source: str = "queue",
            use_cbam: bool = True,
        ...
        self.num_erosion_levels = num_erosion_levels
        if topology_negative_source not in {"queue", "in_batch"}:
            raise ValueError("topology_negative_source must be queue or in_batch")
        self.erosion_kernel_size = erosion_kernel_size
        self.topology_operator = topology_operator
        self.topology_negative_source = topology_negative_source
        ...
        print(
            f"  Topology level maps: {num_erosion_levels} "
            f"(max erosion depth: {num_erosion_levels - 1})"
        )
        print(f"  Erosion kernel: {erosion_kernel_size}")
        print(f"  Topology operator: {topology_operator}")
        print(f"  Topology negatives: {topology_negative_source}")
        print(f"  Backbone: {backbone_name}")
        print(f"  CBAM: {self.use_cbam}")
        print(f"{'=' * 60}\n")

        # ========== 查询编码器（可训练） ==========
        self.encoder_q, resnet_dim = build_feature_backbone(
            backbone_name,
            pretrained=pretrained_backbone,
        )

        # CBAM注意力模块
        self.cbam_q = CBAM(resnet_dim) if self.use_cbam else nn.Identity()
        ...
        # ========== 键编码器（动量更新） ==========
        self.encoder_k, key_dim = build_feature_backbone(
            backbone_name,
            pretrained=False,
        )
        if key_dim != resnet_dim:
            raise RuntimeError("query and key backbone feature dimensions differ")
        self.cbam_k = CBAM(resnet_dim) if self.use_cbam else nn.Identity()
        ...
        self.topo_extractor_q = TopoFeatureExtractor(
            ...
            topology_operator=topology_operator,
        )
    ...
    def contrastive_loss(
        ...
        # ========== 拓扑对比损失 ==========
        if self.topology_negative_source == "queue":
            ...
            topo_logits = torch.einsum('nc,mc->nm', topo_q, topo_k) / self.T
            topo_labels = torch.arange(N, dtype=torch.long, device=topo_q.device)
            topo_loss = F.cross_entropy(topo_logits, topo_labels)
